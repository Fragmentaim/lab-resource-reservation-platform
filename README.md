...
一个面向实验室资源管理场景的前后端分离预约系统。项目覆盖资源浏览、时段预约、热门资源异步确认、预约提醒、未签到自动取消、通知中心和管理后台等功能，重点展示 Spring Boot 后端业务建模、数据库事务、Redis 并发控制、RocketMQ 异步消息与 MQ4 延迟消息实践。
...
- 为什么热门预约要从同步写入改为异步确认。
- Redis 预占和数据库最终确认如何配合，为什么仍要以数据库事务为准。
- outbox 如何解决业务事务和 MQ 投递之间的一致性问题。
- RocketMQ 4.x 延迟等级不支持任意时间时，如何通过自推进重投实现长延迟。
- 消费端为什么要按业务状态做幂等，而不是简单依赖 MQ 不重复投递。
- 死信队列不等于业务兜底，业务侧需要状态机、超时失败和人工/工具化补偿流程配合。
